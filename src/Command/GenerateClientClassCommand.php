...
namespace PhpJit\ApidocClientGenerator\Command;
...
use PhpJit\ApidocClientGenerator\ClientClassGeneratorInterface;
use PhpJit\ApidocClientGenerator\Configuration\AutoloadingStrategy;
use PhpJit\ApidocClientGenerator\Configuration\ComposerConfigurationReaderInterface;
use PhpJit\ApidocClientGenerator\Configuration\Configuration;
use PhpJit\ApidocClientGenerator\GeneratedTestClassDto;
use PhpJit\ApidocClientGenerator\Writer\Psr4ClientClassWriter;
use PhpJit\ApidocClientGenerator\Writer\Psr4ClientClassWriterInterface;
...
class GenerateClientClassCommand extends Command implements GenerateClientClassCommandInterface
{
    protected static $defaultName = 'api:client:generate-classes';
    ...
    private ClientClassGeneratorInterface $testClassGenerator;
    ...
    public function __construct(
        ...
        ClientClassGeneratorInterface $testClassGenerator,
    ...
    {
        $this
            ->setDescription('Generate a Client class from a apiDoc.');
    }
    ...
    private function createTestClassWriter(Configuration $configuration): Psr4ClientClassWriterInterface
    {
        $autoloadingStrategy = $configuration->getAutoloadingStrategy();

        if ($autoloadingStrategy === AutoloadingStrategy::PSR4) {
            return new Psr4ClientClassWriter($configuration);
        }
